...
/* @spec 8.5 §3.1 — apply one revolution's speed-compensated sample.
...
/* @spec 8.5 §3.2 — Sensor deviation: d(s_i) = s_i - mean(s_j for j≠i).
 *
...
/* @spec 8.5 §3.2 — Cylinder deviation:
 *   d(c) = (s_fore + s_aft)/2 - mean(remaining sensors)
 *
...
/* @spec 8.5 §3.3.1 — given thresholds (units = mm hundredths) and a
 * filtered/deviation value (also mm hundredths), classify status. */
slow_wear_status_t slow_wear_classify(int value_mm_hundredths,
                                      int alarm_threshold,
                                      int slowdown_threshold);

/* @spec 8.5 §3.3.2 — pre-warning is checked against the 6-hour average
 * of compensated sensor values, not the per-revolution filtered value.
...
/* @spec 8.5 §3.3.2 — acknowledging a pre-warning raises the limit by
 * 0.25 mm (= 25 hundredths). Returns the new limit. */
