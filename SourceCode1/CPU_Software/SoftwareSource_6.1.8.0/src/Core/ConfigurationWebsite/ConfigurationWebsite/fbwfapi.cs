...
namespace FBWF
{
    ...
    class Fbwfapi
    {
        ...
        public static bool IsDriveProtected(string drive, out bool current, out bool next)
        {
            ...
            uint result = NativeMethods.FbwfIsVolumeProtected(drive, out currentSession, out nextSession);

            logger.DebugFormat("Result of NativeMethods.FbwfIsVolumeProtected {0}", result);
        ...
        public static bool ProtectDrive(string drive)
        {
            if (NativeMethods.FbwfProtectVolume(drive) == NativeMethods.ERROR_SUCCESS)
            {
                return true;
            }
            else
        ...
        public static bool UnprotectDrive(string drive)
        {
            if (NativeMethods.FbwfUnprotectVolume(drive, 1) == NativeMethods.ERROR_SUCCESS)
            {
                return true;
            }
            else
