...
namespace LauncherWarning
{
    public partial class LauncherForm : Form
    {
        ...
        {
            InitializeComponent();
            textBoxWarn.ForeColor =  Color.FromName("Red");
            textBoxWarn.Text = warning;
        }
        ...
        private void buttonRegist_Click(object sender, EventArgs e)
        {
            ...
            string serialNumberFile = @"c:\AMOT\Key\sn.txt";
            ...
            try
            {
                ...
                if (!r.IsMatch(textBoxSerialN.Text.Trim()) || textBoxSerialN.Text.Trim().Length == 0)
                {                                                       
                    MessageBox.Show("Serial number is not valid, please retry!");
                    return;
                }
                ...
                // Delete the file if it exists. 
                if (File.Exists(serialNumberFile))
                {
                    File.Delete(serialNumberFile);
                }

                if (!System.IO.Directory.Exists(@"c:\AMOT\Key\"))
                {
                    System.IO.Directory.CreateDirectory(@"c:\AMOT\Key\");
                }

                //create serial number file
                {
                    // Create the file. 
                    using (FileStream fs = File.Create(serialNumberFile))
                    {
                        Byte[] info = new UTF8Encoding(true).GetBytes(textBoxSerialN.Text.Trim());
