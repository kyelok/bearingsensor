...
namespace ConfigurationDB
{
    ...
    public class ConfigurationDBOperations
    {
        ...
        public void WriteNewRecordToDatabase(string[] values)
        {
            ...
            //TODO:Check that values comforms with database schema
                ...
                using (DbDataReader reader = dbSDK.Select(conn, sql))
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        if (webUpdateDate > spuUpdateDate || forceAll)
                        {
                            ...
                            using (ModbusSDK modbusSDK = new ModbusSDK())
                            {
                                if (modbusSDK.ModbusConnect() == false)
                                {
                                    //Be sure this exception is handled by caller
                                    throw new SlaveException("Failed to connect to modbus server");
                                }
                                //Write new value to modbus
                                modbusSDK.master.WriteSingleRegister(1, register, value);
                            }
