...
namespace Modbus.Device
{
	...
	public abstract class ModbusMaster : ModbusDevice, IModbusMaster
	{
        ...
        /// <param name="numberOfPoints">Number of holding registers to read.</param>
        /// <returns>Holding registers status</returns>
        public ushort[] ReadHoldingRegistersTemp(byte slaveAddress, ushort startAddress, ushort numberOfPoints)
        {
            ValidateNumberOfPoints("numberOfPoints", numberOfPoints, 125);
		...
		public void WriteSingleRegister(byte slaveAddress, ushort registerAddress, ushort value)
		{
            ...
            if (slaveAddress == 0)
            {
                Transport.BroadcastMessage(request);
            }
            else
            {
        ...
        /// <param name="subfunctionCode">The modbus sub function code to use</param>
        /// <param name="data">Data to return.</param>
        /// <returns>Return true if slave device echoed data.</returns>
        public RegisterCollection DiagnosticRequest(byte slaveAddress, ushort subfunctionCode, ushort[] data)
        {
            DiagnosticsRequestResponse request = new DiagnosticsRequestResponse(subfunctionCode, slaveAddress, new RegisterCollection(data));
        ...
        /// <summary>
        /// Gets the number of retries required to complete
        /// the last Modbus transaction
        /// </summary>
        public Int64 RetryCount
        {
            get
            {
                return this.Transport.RetryCount;
            }
        }
        ...
        /// </summary>
        public Int64 CrcErrorCount
        {
            get
            {
                return this.Transport.CrcErrorCount;
            }
        }
