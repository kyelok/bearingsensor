...
namespace MySample
{
	...
	public class Driver
	{
		...
		public static void StartModbusTcpSlave()
		{
            ...
            //slave.DataStore.HoldingRegisters[0,1,2] = 1; //BCD
            ...
            do
            {
                ...
                lock (slave.DataStore.SyncRoot)
                {
                    ...
                    // Data and time
                    slave.DataStore.HoldingRegisters[0] = (ushort)((toBCD(time.Year - 2000) << 8) + toBCD(time.Month));
                    slave.DataStore.HoldingRegisters[1] = (ushort)((toBCD(time.Day) << 8) + toBCD(time.Hour));
                    slave.DataStore.HoldingRegisters[2] = (ushort)((toBCD(time.Minute) << 8) + toBCD(time.Second));
                }

                slave.DataStore.HoldingRegisters[125]++;
                slave.DataStore.HoldingRegisters[126]++;

                slave.DataStore.HoldingRegisters[250]++;
                slave.DataStore.HoldingRegisters[251]++;

                slave.DataStore.HoldingRegisters[375]++;
                slave.DataStore.HoldingRegisters[376]++;

                slave.DataStore.HoldingRegisters[500]++;
                slave.DataStore.HoldingRegisters[501]++;
        ...
        public static ushort toBCD(int value)
        {
            return (ushort)(((value / 10) << 4) | (value % 10));
        }
