...
namespace Modbus.IO
{
	...
	public abstract class ModbusTransport : IDisposable
	{
        ...
        /// <summary>
        /// Returns the number of crc errors that have occured
        /// The number of errors are reset and getting this value
        /// </summary>
        public int CrcErrorCount
        {
            get
            {
                int r = this._crcErrorCount;
                this._crcErrorCount = 0;
                return r;
            }
        ...
        /// <summary>
        /// Returns the number of retries that have occured
        /// The number of errors are reset and getting this value
        /// </summary>
        ...
        internal void BroadcastMessage(IModbusMessage message)
        {
            try
            {
                lock (_syncLock)
            ...
            catch (Exception e)
            {
                if (e is FormatException ||
                    ...
                    e is InvalidCRCException ||
                    e is SlaveException)
                {
                    //Ignore these exceptions
                    //there is nothing we can do about them when broadcasting
                }
                else
                {
                    throw;
			...
			do
			{
				...
				catch (SlaveException se)
				{
                    if (se.SlaveExceptionCode != Modbus.SlaveDeviceBusy)
						throw;
					_logger.InfoFormat("Received SLAVE_DEVICE_BUSY or GATEWAY_TARGET_DEVICE_FAILED_TO_RESPOND exception response, waiting {0} milliseconds and resubmitting request.", _waitToRetryMilliseconds);
					Thread.Sleep(WaitToRetryMilliseconds);
				}
				catch (Exception e)
				{
                        ...
                        e is SlaveException)
					{
                        ...
                        if (e is SlaveException)
                        {
                            //If not a Gateway timeout response rethrow the exception
                            //If it is a Gateway timeout response we want to fall throw
                            //to preform a retry.
                            if (((SlaveException)e).SlaveExceptionCode != Modbus.SlaveGatewayTargetDeviceFailedToRespond)
                            {
                                throw;
                            }
                        }

                        if (e is InvalidCRCException)
                            this._crcErrorCount++;
                        ...
                        {
                            if((attempt) > 1) 
                                _logger.WarnFormat("{0}, {1} retries remaining", e.GetType().Name, _retries - attempt);
                        }
                        else
                        {
		...
		internal void ValidateResponse(IModbusMessage request, IModbusMessage response)
		{
			...
			if (request.FunctionCode != response.FunctionCode)
                throw new FormatException(String.Format(CultureInfo.InvariantCulture, "Received response with unexpected Function Code. Expected {0}, received {1}.", request.FunctionCode, response.FunctionCode));

			if (request.SlaveAddress != response.SlaveAddress)
                throw new FormatException(String.Format(CultureInfo.InvariantCulture, "Response slave address does not match request. Expected {0}, received {1}.", response.SlaveAddress, request.SlaveAddress));
