...
## Behavioural changes from bug fixes

Several long-standing bugs in the v6.20 firmware have been corrected. Some are visible to operators; please review before commissioning.

### Pre-warning averaging window now matches operator setting (Lv-010)

**What was happening**: the pre-warning algorithm computes a rolling average of sensor BDC values over an operator-configured window (default 360 minutes / 6 hours) and alarms when the live value drifts outside expected bounds relative to that average. Due to a 16-bit-integer arithmetic overflow on the C2000 chip, the timer that gated the averaging window only ran for **~5 minutes** instead of the configured 6 hours.

**What changes after the fix**: the averaging window is now the configured value. With the default 360 minutes:
...
- Pre-warnings will now reflect drift relative to a longer-term trend, not against a fast-changing reference.

**What to watch for after install**:
- During the first 6 hours after a fresh install, pre-warning behaviour may be different from prior installations. Allow a full averaging cycle before treating any pre-warning as actionable.
- If the prior 5-minute behaviour had been informally calibrated against (e.g. operators tuning thresholds based on that timing), thresholds may need light re-tuning. AMOT field service can advise.

**Modbus register**: the configured window in minutes is at the existing `prewarningtimer` register; behavior is unchanged as far as configuration goes — only the actual timer math is now correct.

### Other patched bugs

- **Bug Lv-005** (calibration restart buffer overflow): fixed. Previously, restarting calibration after the system had run for a while could silently corrupt adjacent globals — most often manifesting as unexplained changes in alarm thresholds or counter values immediately after a re-cal. Operators may have observed this as "the cal cleared more than I expected." That class of artefact is now eliminated.
- **Bug Lv-008** (Modbus parameter table off-by-one): fixed. Reading an unrecognised Modbus register no longer reaches past the parameter array. Customer-visible symptom (rare): unrecognised register reads returning garbage instead of cleanly responding "register not supported."
- **Bug 2** (firmware version display in `system_info.csv`): fixed in the **CPU dashboard** (not the SPU firmware). The exported "SPUFirmware" field used to show "6.32" for firmware v6.20 and an empty string for round versions like v7.00. After CPU dashboard rebuild, the field will show the correct human-readable version (e.g. "6.20", "7.00", "8.70").
- **Bug 3** (`KValueDamageMonitoring` showing as a negative number): fixed in the **CPU dashboard**. The CSV export incorrectly cast unsigned Modbus register values to signed 16-bit, producing negative numbers for K-value settings above 32767. After CPU dashboard rebuild, the value displays correctly.

> Note: Bugs 2 and 3 require a rebuild of the **Windows dashboard software** (Silverlight); the SPU firmware itself was never affected. Coordinate with the dashboard team for the dashboard release timing.
...
- **The `system_info.csv` fields** "firmware version" and "KValueDamageMonitoring" — patched in the dashboard source tree (see "Behavioural changes" above). After the next dashboard release, both fields display correctly.
